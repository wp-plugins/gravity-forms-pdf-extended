...
class GFPDF_InstallUpdater
{
	...
	{	
		 ...
		 if($deploy && PDF_DEPLOY === true)
		 {
			 ...
			 foreach(glob(PDF_PLUGIN_DIR.'templates/*.php') as $file) {
				 	$path_parts = pathinfo($file);					
						if(file_exists(PDF_TEMPLATE_LOCATION.$path_parts['basename']))
						{
							unlink(PDF_TEMPLATE_LOCATION.$path_parts['basename']);
						}
			 }
		...
		if(!is_dir(PDF_FONT_LOCATION))
		{
			/* create new directory in active themes folder*/	
			if(mkdir(PDF_FONT_LOCATION) === false)
			{
				add_action('admin_notices', array("GFPDF_InstallUpdater", "gf_pdf_template_dir_err")); 	
				return;
			}
		...
		{ 
			...
			if(!copy(PDF_PLUGIN_DIR .'configuration.php', PDF_TEMPLATE_LOCATION.'configuration.php'))
			{ 
				add_action('admin_notices', array("GFPDF_InstallUpdater", "gf_pdf_template_dir_err")); 	
				return;
			}
	...
	{		
		if( (PDF_DEPLOY === true) && !rgpost('cancel') && !rgpost('update') )
		{
	...
	public function pdf_extended_copy_directory( $source, $destination, $copy_base = true ) 
	{
		if ( is_dir( $source ) ) 
		{
			if($copy_base === true)
