 ...
 * We aren't including these fonts in the main package.
 * So we need to check if users want to include them by adding them to the font folder.
 * Check if CJK fonts exists and move them to the fonts folder
 * This prevents them being removed on upgrade.
 */
if(file_exists(PDF_FONT_LOCATION.'UnBatang.ttf'))
{
	if(!file_exists(PDF_PLUGIN_DIR.'mPDF/ttfonts/UnBatang.ttf'))
	{
		copy(PDF_FONT_LOCATION.'UnBatang.ttf', PDF_PLUGIN_DIR.'mPDF/ttfonts/UnBatang.ttf');	
	}

	$this->fontdata['unbatang'] = array(
				'R' => "UnBatang.ttf"
	);		
}

if(file_exists(PDF_FONT_LOCATION.'Sun-ExtA.ttf'))
{
	if(!file_exists(PDF_PLUGIN_DIR.'mPDF/ttfonts/Sun-ExtA.ttf'))
	{
		copy(PDF_FONT_LOCATION.'Sun-ExtA.ttf', PDF_PLUGIN_DIR.'mPDF/ttfonts/Sun-ExtA.ttf');	
	}
		
	$this->fontdata['sun-exta'] = array(
				'R' => "Sun-ExtA.ttf"
	);
}

if(file_exists(PDF_FONT_LOCATION.'Sun-ExtB.ttf'))
{
	if(!file_exists(PDF_PLUGIN_DIR.'mPDF/ttfonts/Sun-ExtB.ttf'))
	{
		copy(PDF_FONT_LOCATION.'Sun-ExtB.ttf', PDF_PLUGIN_DIR.'mPDF/ttfonts/Sun-ExtB.ttf');	
	}
	
	$this->fontdata['sun-extb'] = array(
				'R' => "Sun-ExtB.ttf"
	);
}
