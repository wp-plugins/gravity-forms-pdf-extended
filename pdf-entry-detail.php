...
if(!class_exists('GFPDFEntryDetail'))
{
	class GFPDFEntryDetail {
		...
		/* NEED THIS FUNCTION - BLD */
		public static function lead_detail_grid($form, $lead, $allow_display_empty_fields=false, $show_html=false){
			$form_id = $form["id"];
						...
						switch(RGFormsModel::get_input_type($field)){
						   case "section" :
							...
							default:
								...
								if($display_empty_fields || !empty($display_value) || $display_value === "0" || $allow_display_empty_fields){
									$count++;	
									$is_last = $count >= $field_count && !$has_product_fields ? true : false;
									$last_row = $is_last ? " lastrow" : "";
									$even = ($count%2) ? ' odd' : ' even';
						...
						switch(RGFormsModel::get_input_type($field)){
							...
							case "signature":
								...
								if(file_exists($folder.$value) !== false)
								{
									...
									$form_array['signature_details'][] = array('img' => '<img src="'. $http_folder.$value .'" alt="Signature" width="100" height="60" />',
																	   'path' => $http_folder.$value);
								}
